...
func runInstall(
	...
	// 4. SCM failure recovery（+  failureflag — 失败仅 warn；
	//    service.go samesession=false + supervisor self-swap exit(0) 依赖此配置触发 SCM restart）
	if err := sc.ConfigureRecovery(); err != nil {
		log.Warn("failed to configure SCM failure recovery (non-fatal; supervisor won't auto-restart on crash or graceful exit)",
			slog.String("err", err.Error()))
	}
