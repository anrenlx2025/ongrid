...
type geminioClient struct {
	...
	// endOpts is the EndOptions pointer passed to NewRetryEndWithDialer.
	// RetryEnd stores this pointer internally and re-uses it on every
	// reconnect, so calling endOpts.SetMeta() updates the auth blob
	// used on the next reconnect without closing the current End.
	//
	// credMu guards the credential fields (cfg.AccessKey/SecretKey) and
	// endOpts: UpdateCredentials may be called from another goroutine
	// while Dial reads them to build the auth meta.
	credMu  sync.Mutex
	endOpts *gclient.EndOptions
...
// tunnel authentication on subsequent reconnects. RetryEnd stores the
// *EndOptions pointer internally, so calling SetMeta here updates the
// auth blob transparently — the next reconnect (automatic or manual)
// will use the new credentials. The current session stays authenticated
// via the old credentials until the connection drops.
func (c *geminioClient) UpdateCredentials(accessKey, secretKey string) {
	c.credMu.Lock()
	defer c.credMu.Unlock()
	c.cfg.AccessKey = accessKey
	c.cfg.SecretKey = secretKey
	if c.endOpts == nil {
		return
	}
	meta, err := json.Marshal(Meta{
	...
	if err != nil {
		c.log.Warn("tunnel: UpdateCredentials marshal meta failed",
			slog.Any("err", err))
		return
	}
	c.endOpts.SetMeta(meta)
	c.log.Info("tunnel: credentials updated for next reconnect")
}
