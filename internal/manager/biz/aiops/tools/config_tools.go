        ...
        "conditions": {
          ...
          "items": {"$ref": "#/$defs/condition"}
	        ...
	        "spec": {
	          ...
	          "description": "Kind-specific spec. log_search uses keywords={include:[],exclude:[],mode:any|all|phrase}, scope={device_ids,cluster_ids,namespaces,workloads,pods,containers,service_names,source_ids,severities,files}, optional allowlisted filters, window, operator, and threshold. Never generate LogQL or Elasticsearch DSL for log_search. metric_raw accepts expr/promql/query as a full boolean PromQL predicate, or metric plus operator and threshold for one exact Prometheus metric. Use metric names and label keys from list_metric_catalog when available. Only scope selectors when the user explicitly asked for that source/device/job/service/instance; mark that with source_explicit=true. Other kinds use their natural fields: metric_anomaly, metric_forecast, metric_burn_rate, legacy log_match/log_volume, trace_latency, trace_error_rate."
	        },
