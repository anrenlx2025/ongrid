  ...
  "$defs": {
    "rule": {
      ...
      "properties": {
        ...
        "name": {"type": "string"},
        "scope_type": {"type": "string", "enum": ["global", "host", "monitoring_pipeline"], "description": "log_search is always global and produces one aggregate incident. Use host for metric rules associated with a specific machine or device-collected instance, such as CPU, memory, disk/filesystem, load, network, database/Redis/MongoDB metrics, or when the final PromQL result keeps a device_id label. Use global for log_search, service, SLO, trace, or intentionally aggregated fleet-wide rules."},
        ...
        "conditions": {
          ...
          "items": {"$ref": "#/$defs/condition"}
	        ...
	        "spec": {
	          ...
	          "description": "Kind-specific spec. log_search uses keywords={include:[],exclude:[],mode:any|all|phrase}, scope={device_ids,cluster_ids,namespaces,workloads,pods,containers,service_names,source_ids,levels,files}, optional allowlisted filters, window, operator, and threshold. Never generate LogQL or Elasticsearch DSL for log_search. metric_raw accepts expr/promql/query as a full boolean PromQL predicate, or metric plus operator and threshold for one exact Prometheus metric. Use metric names and label keys from list_metric_catalog when available. Only scope selectors when the user explicitly asked for that source/device/job/service/instance; mark that with source_explicit=true. Other kinds use their natural fields: metric_anomaly, metric_forecast, metric_burn_rate, trace_latency, trace_error_rate."
	        },
